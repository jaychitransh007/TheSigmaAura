...
> **OPS CONTEXT (May 18, 2026) — Shopify-hosted system.** The SQL panels below query the engine's `public` schema tables and remain valid for engine-side observability of both the legacy web channel and the new `vibe_storefront` channel. What's where:
>
...
> - **Vibe Shopify App** (Vercel `bom1`) — runtime logs via `vercel logs --environment production --no-branch -x -n 60`. Errors, function timing, cold-start metrics in the Vercel dashboard. The Remix actions emit structured JSON events to stdout (captured by Vercel Logs + any drain) via [`vibe-app/app/lib/logger.server.ts`](../vibe-app/app/lib/logger.server.ts):
