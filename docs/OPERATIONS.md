...
> **OPS CONTEXT (May 18, 2026, evening) — Shopify-hosted system.** The SQL panels below query the engine's `public` schema tables and remain valid for engine-side observability of both the legacy web channel and the new `vibe_storefront` channel. What's where:
>
...
> - **Vibe Shopify App** (Vercel `bom1`) — installed in production on TheSigmaVibe (`q8pery-95.myshopify.com`) as of 2026-05-18 evening. Runtime logs via `vercel logs --environment production --no-branch -x -n 60`. Errors, function timing, cold-start metrics in the Vercel dashboard. The Remix actions emit structured JSON events to stdout (captured by Vercel Logs + any drain) via [`vibe-app/app/lib/logger.server.ts`](../vibe-app/app/lib/logger.server.ts):
>   - **Onboarding / chat** (May 17): `event=vibe_init_outcome` (three-way `getOnboardingStatus` result + ensure-profile outcome + derived `has_profile`), `event=vibe_merge_outcome`, `event=vibe_merge_identity_mismatch` (warn — 403 IDOR rejection), `event=vibe_turn_pairing_rewrite` (silent "Build an outfit around this attached piece" augmentation).
>   - **Catalog ingest + webhooks** (May 18 morning): `event=vibe_bootstrap_tick`, `event=vibe_cron_daily_sync_*`, `event=vibe_product_webhook_*`.
>   - **Theme inheritance + menu injection** (May 18 afternoon): `event=vibe_theme_overrides_refresh{outcome=applied|skipped_unchanged|skipped_empty_probe|failed}` — applied case carries `has_logo`, `has_font`, `has_color_primary`, `menu_items_count` for coverage dashboards. `event=vibe_menu_injection_outcome{outcome=added|already_present|skipped|failed}` — slice rollout coverage + scope-grant problems. `event=vibe_navigation_menu_lookup_failed` / `vibe_navigation_menu_update_failed` with `stage` labels (index / detail / http / user_errors / exception). `event=vibe_tenant_lookup_failed`.
>   - **Onboarding stage + failure logs** (May 18 evening, [PR #501](https://github.com/jaychitransh007/TheSigmaAura/pull/501)): `event=vibe_onboarding_stage_reach{stage="photo_uploaded|profile_field_saved|analysis_triggered", sessionId, ...}` — success-path funnel events from the three onboarding proxy routes (image / profile / analysis). Use `COUNT(DISTINCT sessionId) BY stage` for funnel shape. `event=vibe_onboarding_endpoint_failed{endpoint="image|profile|analysis", status, error, sessionId, ...}` — paired error-path event so silent vibe-app → engine failures (the class behind the May-18-evening 49-minute stuck-spinner incident) are now grep-able.
...
> - **Prometheus metrics** — engine exposes `/metrics`. May-16 → May-18 additions:
...
>   - F.2.x catalog sweep (#475/#476): `aura_outfit_item_variant_ids_missing_total`, `aura_bootstrap_batch_total{status}` + per-outcome histogram, `aura_product_webhook_total{topic, status}`, `aura_enrichment_batch_total{status}` + `aura_enrichment_rows_{ingested,failed}_total`, `aura_catalog_status_change_total{action}`.
>   - Theme inheritance (#498): `aura_theme_overrides_patch_total{outcome="applied|error"}` — every PATCH /v1/tenants/{id}/theme-overrides, triggered by admin home loader OR themes/update webhook. Pairs with the Vibe-side `vibe_theme_overrides_refresh` event for full-stack tracing.
>   - Tenant lookup + catalog linkage ([#502](https://github.com/jaychitransh007/TheSigmaAura/pull/502), May 18 evening): `aura_tenant_lookup_total{outcome="created|existing|error"}` — every POST /v1/tenants/lookup-or-create, attributes first-install vs idempotent admin-home refresh. `aura_catalog_shopify_linkage_observed_total{bucket="fully_linked|partially_linked|mostly_unlinked"}` — fires on POST /v1/tenants/{id}/bootstrap-complete (future: F.3 cron) capturing what fraction of the tenant's `catalog_enriched` rows have `shopify_product_id` populated. Pairs with structured log `event=catalog_shopify_linkage_observed` (`tenant_id`, `total_rows`, `linked_rows`, `linkage_rate_pct`, `bucket`) — `tenant_id` is intentionally in the log (not as a Prom label) to avoid high-cardinality blow-up.
...
>   - See `docs/OPEN_TASKS.md` § Trigger-driven for remaining low-priority gaps (variant-id coverage histogram, `tenant_id` in per-match logs of `catalog_search_agent._hydrate_matches`).
>
