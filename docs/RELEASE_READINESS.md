...
Last updated: May 18, 2026 (evening).

> **RELEASE TARGET (May 18, 2026).** The 4 gates below describe the readiness criteria for the *legacy standalone Aura web app* (port 8010 server-rendered HTML chat). The Shopify-hosted system has shipped on top of that and is now multi-tenant + installed in production:
>
...
> - **D.P.1 install shipped (May 18 evening).** Vibe installed on TheSigmaVibe production store (`q8pery-95.myshopify.com`) via Partner-Dashboard custom-distribution. Tenant row pre-seeded `bootstrap_status='ready'` (migration `20260518000000`), so `SyncProgressCard` correctly short-circuits on first admin home load — no install-time re-enrichment of the 13,177 pre-existing catalog rows. `shopify app deploy` ran first to register the six scopes + F.4 webhook subscriptions in Partner Dashboard.
> - **B.8 GID capture shipped (May 18 evening, handle-suffix variant).** Original `capture_shopify_gids.py` matched zero of 12,199 Shopify products because Shopify silently dropped the `vibe.source_product_id` metafield column during CSV import (the import only honours metafield columns when a matching metafield definition pre-exists in the store). New `capture_shopify_gids_by_handle.py` matches via the trailing 10-char `pid_suffix` that `build_handle` encodes into every Shopify handle — 11,977 of 12,199 products linked cleanly. 222 stragglers accepted (208 ambiguous duplicate rows in `catalog_enriched` from earlier ingestion; 14 Campus Sutra products with SKU-style trailing tokens). Expected enrichment cost when their next webhook/cron event fires: ~$0.67 total.
> - **Conversation page live** at `/apps/vibe/style` at full feature parity with the legacy `platform_core/ui.py` reference. In-chat onboarding flow (D.O.1–D.O.3): branched welcome, parallel photos + gender-DOB cards, HEIC handling, photo zoom/pan, combined ft/in height-waist card, post-onboarding analysis indicator + templated "what would you like to wear?" prompt. **Analysis-trigger self-heal** (PR #500, May 18 evening) — the fire-and-forget POST to `/v1/onboarding/analysis/start-phase{1,2}` now retries from the poll loop when status sits at `not_started`, with an 8s cooldown. Caught after a real customer's session sat at "Analyzing your style…" for 49 minutes because the original `.catch(() => {})` swallowed an HTTP error.
> - **Add-to-Cart end-to-end working** (PR #474): response_formatter now passes `shopify_variant_ids` + `shopify_product_id` + `catalog_description` through to the storefront. PR #475 added a regression guard (`aura_outfit_item_variant_ids_missing_total` + WARNING log) so this bug class can't reappear silently. Activates on TheSigmaVibe the moment B.8's linked rows surface as outfits.
> - **Shopify Customer Account merge** (D.S.3b) wired via `POST /v1/users/merge`, IDOR-guarded. Attribution propagation fix (PR #503, May 18 evening): the merge now also copies `acquisition_source` / `acquisition_campaign` / `referral_code` / `icp_tag` from the alias's `onboarding_profiles` to the canonical's when the canonical is at defaults — without it, a vibe_storefront customer who later signs in via Shopify Customer Account would show up in Panel 01 as `unknown` forever.
> - **Observability** — engine: `aura_turn_total{channel}`, `aura_user_merge_total{status}`, `aura_onboarding_endpoint_total{endpoint, status, channel}`, `aura_onboarding_image_bytes{category, channel}`, plus the F.2.x sweep added in #475/#476 (`aura_outfit_item_variant_ids_missing_total`, `aura_bootstrap_batch_total{status}` + per-outcome histogram, `aura_product_webhook_total{topic, status}`, `aura_enrichment_batch_total{status}` + row-count totals, `aura_catalog_status_change_total{action}`), plus #498 (`aura_theme_overrides_patch_total{outcome}`), plus #502 (`aura_tenant_lookup_total{outcome=created|existing|error}`, `aura_catalog_shopify_linkage_observed_total{bucket=fully_linked|partially_linked|mostly_unlinked}` + structured log `catalog_shopify_linkage_observed` with `tenant_id`). Vibe: JSON-to-stdout structured logs (`vibe_init_outcome`, `vibe_merge_outcome`, `vibe_turn_pairing_rewrite`, `vibe_bootstrap_tick`, `vibe_cron_daily_sync_*`, `vibe_product_webhook_*`, `vibe_theme_overrides_refresh`, `vibe_menu_injection_outcome`, `vibe_navigation_menu_{lookup,update}_failed`, `vibe_tenant_lookup_failed`, plus #501 `vibe_onboarding_stage_reach{stage=photo_uploaded|profile_field_saved|analysis_triggered}` and `vibe_onboarding_endpoint_failed{endpoint=image|profile|analysis, status, error}`) via [`vibe-app/app/lib/logger.server.ts`](../vibe-app/app/lib/logger.server.ts).
...
> - **Remaining release blockers** for first-50 rollout, tracked in [`OPEN_TASKS.md`](OPEN_TASKS.md) § Next priorities: **B.4** (mandatory legal pages — drafts ready, awaiting counsel review), **B.6** (real-card test order — manual), **D.C.9** (one Buy Outfit click on prod to confirm `/cart/add.js` resolves to real variant ids now that B.8's mapping is in place).
>
...
## Recently Shipped — May 18, 2026 — evening push (D.P.1 install + B.8 GID capture + observability hardening)

The final push of the day got Vibe installed on TheSigmaVibe production store, linked the imported catalog rows to their Shopify product IDs, and closed three observability gaps surfaced during install.

### D.P.1 — Vibe installed on TheSigmaVibe production
- **Path used:** Partner-Dashboard custom-distribution. Dev Dashboard's "Develop apps" flow (legacy in-Admin path) is gone in newer stores like The Vibe Shop, replaced by the Dev Dashboard surface. Custom-distribution apps installed via the OAuth `install_custom_app` URL flow issue `shpca_*` access tokens (not the legacy `shpat_*` shape), reachable through the same `X-Shopify-Access-Token` header on Admin GraphQL.
- **Pre-install validation:** tenant row for `q8pery-95.myshopify.com` is pre-seeded in migration `20260518000000_f20_f21_tenants_and_retrieval.sql` with `bootstrap_status='ready'` and `product_count=13177`. The `SyncProgressCard` `useEffect` short-circuits on `bootstrap_status in {ready, failed}` ([app._index.tsx:652](../vibe-app/app/routes/app._index.tsx:652)), so install-time bootstrap never auto-fires against an already-bootstrapped tenant.
- **Webhook subscriptions registered:** `shopify app deploy` from `vibe-app/` pushed the current `shopify.app.vibe.toml` to Partner Dashboard, registering `products/{create,update,delete}` + `themes/update` + the three GDPR privacy-compliance topics, plus the six scopes (`read_products`, `read_inventory`, `read_orders`, `read_customers`, `read_themes`, `write_online_store_navigation`).
- **Frozen-catalog policy honoured:** no LLM call fired on the pre-existing 13,177 enriched rows. Install-time bootstrap and F.4 webhooks both cache-hit on `(tenant_id, shopify_product_id)` once B.8 lands the join keys (below).

### B.8 — Shopify GID capture (handle-suffix variant)
- **Why a variant:** the original `scripts/seed_thesigmavibe_catalog/capture_shopify_gids.py` matches Shopify products to `catalog_enriched` rows via the `vibe.source_product_id` metafield. Live diagnostic against the production store showed every Shopify product missing that metafield — Shopify's CSV import silently dropped the metafield column because no matching metafield definition pre-existed in the store at import time. Matched products: 0 of 12,199.
- **The fix:** new `scripts/seed_thesigmavibe_catalog/capture_shopify_gids_by_handle.py` matches on the last 10 characters of the kebab-cased `product_id`, which `build_handle` already encodes as the trailing segment of every Shopify handle (`fawn24-buttercup-striped-shirt-1312537831` → `_kebab("Fawn24_9053196386535_50491312537831")[-10:]` = `1312537831`). Pulls all `catalog_enriched.product_id` values for the tenant into a suffix-keyed dict, walks Shopify with the same Admin GraphQL pagination, PATCHes `shopify_product_id` + `shopify_variant_ids` back onto the matched row.
- **Outcome:** 11,977 / 12,199 products linked cleanly (98.2%). 208 ambiguous matches (Campus Sutra catalog has duplicate rows for the same logical product — `44903385858306` and `CAMPUSSUTRA_44903385858306` both exist with identical title + price) and 14 unmatched (Campus Sutra SKU-style suffixes the simple `handle.rsplit("-", 1)[-1]` extraction missed; a `handle[-10:]` fix would recover them but the user accepted the ~$0.67 enrichment cost when those products surface via cron/webhook).
- **Frozen-catalog SQL invariant verified:** PATCH only writes `shopify_product_id` and `shopify_variant_ids`; `row_status` stays `'ok'` for all 13,177 rows, so the vision enrichment service's `WHERE row_status='pending_enrichment' AND vision_batch_id IS NULL` filter ([catalog_vision_enrichment_service.py:209](../modules/agentic_application/src/agentic_application/services/catalog_vision_enrichment_service.py:209)) skips them entirely.

### Analysis-trigger self-heal (PR #500)
- **The bug:** customer's session sat at "Analyzing your style…" for 49 minutes. Engine logs showed only GET status reads for the user; no POST to `/v1/onboarding/analysis/start-phase{1,2}` ever landed. Root cause in [apps.vibe.style.tsx](../vibe-app/app/routes/apps.vibe.style.tsx): `handleOnboardingPhotoUploaded` posted to `/apps/vibe/api/onboarding/analysis` with `.catch(() => {})`, which only catches network errors. An HTTP 4xx/5xx response resolves the promise normally — the customer's onboarding-time trigger silently failed, no retry, the poll loop saw `not_started` forever.
- **The fix:** poll loop tick now detects `data.status === "not_started"` and re-fires both phase1 + phase2 with an 8s `analysisTriggerCooldownRef` throttle. Engine treats duplicate start calls idempotently (verified live: triggering both phases while phase1 was already running returned 200 without restarting). Recovery scenarios covered: original trigger fetch returned 4xx/5xx; customer reloaded the page mid-flight; engine rejected on a transient race with image moderation.

### Onboarding stage-reach + endpoint-failed logging (PR #501)
- **Three vibe-app onboarding routes** ([analysis.tsx](../vibe-app/app/routes/apps.vibe.api.onboarding.analysis.tsx), [image.tsx](../vibe-app/app/routes/apps.vibe.api.onboarding.image.tsx), [profile.tsx](../vibe-app/app/routes/apps.vibe.api.onboarding.profile.tsx)) gained two structured-log events:
  - `vibe_onboarding_stage_reach{stage="photo_uploaded|profile_field_saved|analysis_triggered", ...}` — success path, carries route-specific context (`category` | `field` | `phase`) + `sessionId`. Unblocks the funnel analytics OPEN_TASKS.md flagged as "trigger: after first real customer cohort flows through /apps/vibe/style" — D.P.1 fired that trigger today.
  - `vibe_onboarding_endpoint_failed{endpoint, status, error, ...}` — error path. Vercel access logs already capture HTTP status; this event additionally carries the engine's error message body so the silent-fail class behind the 49-min stuck spinner is now grep-able.

### Tenant lookup + catalog linkage counters (PR #502)
- **`aura_tenant_lookup_total{outcome}`** — outcomes `created` (first install), `existing` (idempotent admin-home refresh), `error`. Wired into `lookup_or_create_tenant` ([api.py](../modules/agentic_application/src/agentic_application/api.py)) via a pre-existence check that attributes the outcome before calling the (idempotent) `get_or_create`. One extra `select` on a low-volume endpoint is acceptable for the disambiguation.
- **`aura_catalog_shopify_linkage_observed_total{bucket}`** — `fully_linked` (≥99%), `partially_linked` (≥50 and <99%), `mostly_unlinked` (<50%). Paired with a structured log `catalog_shopify_linkage_observed` carrying `tenant_id` + raw counts. Wired into `bootstrap_complete` via a single column-narrow `select` on `catalog_enriched`. `tenant_id` lives in the structured log (not as a Prom label) to avoid high-cardinality metric blow-up.
- **Future callers** (F.3 daily cron, an admin on-demand endpoint) can call `observe_catalog_linkage(...)` without further engine changes.

### Attribution propagation on merge (PR #503)
- **The bug:** `ConversationRepository.merge_external_user_identity` reassigns conversations and four history tables from alias → canonical, but never touches `onboarding_profiles`. The canonical's `acquisition_source` was created with the default `"unknown"` by `get_or_create_user`; the alias's row carried `"vibe_storefront"`. After merge, Panel 01 would show the customer as `unknown` forever.
- **The fix:** new private `_propagate_attribution(alias_user_id, canonical_user_id)` helper reads both `onboarding_profiles` rows; for each of `acquisition_source`, `acquisition_campaign`, `referral_code`, `icp_tag`, copies alias→canonical IFF alias has a non-default value AND canonical is at default. Best-effort: any exception is logged and swallowed so attribution-table issues never block the merge itself.
- **Three new tests** cover the happy-path propagation, the no-overwrite-when-canonical-already-attributed case, and the no-op-when-alias-profile-missing case. All 60 `tests/test_platform_core.py` tests pass.

### What's next (evening push)
Launch-blocking chain narrows to: **B.4** (legal-page review + publish), **B.6** (real-card test order), **D.C.9** (one Buy Outfit click on prod to verify add-to-cart resolves to real variant ids). Beyond that — Phase G (order webhooks + attribution) and the D.M.* admin pages.
