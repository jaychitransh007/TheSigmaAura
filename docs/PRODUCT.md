...
> **CURRENT SYSTEM (May 16, 2026).** Aura ships as a **3-piece Shopify-hosted product**:
>
...
> 2. **Vibe (Shopify App)** — deployed to Vercel at `https://vibe-app-five.vercel.app` (Hobby plan, functions pinned to `bom1`). Customer-facing chat runs at `thesigmavibe.shop/apps/vibe/style` via App Proxy with HMAC validation. Identity flows via localStorage UUID for anonymous customers; Shopify Customer Account merge folds anonymous history into `shopify:{customer_id}` on sign-in (`POST /v1/users/merge`). In-chat onboarding (photos + gender/DOB + name/height/waist) collects profile data with full Skip support and persistence across reloads. Merchant admin (Polaris) shares the same Remix app.
> 3. **Vibe Engine** — `platform_core` Python service deployed to **Fly.io Mumbai** (`vibe-engine.fly.dev`, single shared-cpu-1x always-on, `/healthz` checked every 30s). The `vibe_storefront` channel bypasses the onboarding gate so anonymous / partial-profile customers can chat; the legacy `web` channel keeps the strict path. Multi-tenant refactor (Phase C) deferred until Vibe customer-page UX validates.
>
