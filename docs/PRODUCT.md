...
- web UI: Confident Luxe design system (Phase 14) + intent-organized discovery surface (Phase 15 — complete) — ivory/oxblood/champagne palette, Fraunces + Inter + JetBrains Mono, hairline borders, full dark mode; 5-tab nav (Home / Outfits / Checks / Wardrobe / Saved) with 56px header. **Tab page headers removed (May 12 2026)** — the tab nav itself is the breadcrumb; each tab opens straight onto content. Home = discovery input + PDP carousel with CSS slide transitions, swipe/keyboard nav, iteration stacking. **Outfits = occasion-driven theme buckets (Beach & Vacation / Date Night / Office & Professional / Party & Night Out / Wedding & Engagement / Festival / Weekend & Everyday / Travel) + formality fallback (Smart Looks / Easy Everyday / Off-Duty) for sessions with no occasion signal — driven server-side from the planner's occasion + the rater's `formality_pct`.** Each theme renders a single flattened carousel of every look in that bucket (no nested per-session carousels). Single source filter (All / From Wardrobe / Hybrid / Shop). Checks = outfit check cards. **Outfit card right column is a context-sync'd detail panel (PR #306)** — try-on thumbnail active → outfit mode (title + reasoning + per-garment rows with price + XS-XL size chips + disabled Buy Outfit); garment thumbnail active → garment mode (title + ×1.2 marked-up price + composer-authored `description` + size chips + Buy Now + Save). User-facing rater radar retired; rater still runs server-side for ranking/filtering. All legacy surfaces removed: no chat bubbles, no conversation sidebar, no Trial Room tab, no Looks page. Feedback redesigned as contextual strip (heart + "What would you change?" inline expansion). Native browser `alert`/`confirm` replaced with in-app `auraPrompt` modal helper (ESC-dismissable).
...
- 3-column PDP outfit cards (thumbnails | hero | context-sync'd detail panel, PR #306) — detail panel mode follows the active thumbnail (try-on → outfit mode with reasoning + per-garment price/size rows + disabled Buy Outfit; garment → garment mode with composer-authored `description` + ×1.2 price markup + size chips + Buy Now + Save). Composer (`outfit_composer.md`) now writes one sentence per garment into `item_descriptions: {item_id: str}`, threaded through ComposedOutfit → orchestrator → response_formatter into `OutfitItem.description`. User-facing rater radar retired May 12 2026; rater still runs server-side for ranking/filtering (`fashion_score_pct`, `formality_pct`, etc. populate on `EvaluatedRecommendation` but are not displayed).
