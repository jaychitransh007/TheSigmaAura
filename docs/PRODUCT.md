...
> **CURRENT SYSTEM (May 17, 2026).** Aura ships as a **3-piece Shopify-hosted product**:
>
...
> 2. **Vibe (Shopify App)** — deployed to Vercel at `https://vibe-app-five.vercel.app` (Hobby plan, functions pinned to `bom1`). Customer-facing chat runs at `thesigmavibe.shop/apps/vibe/style` via App Proxy with HMAC validation. Identity flows via localStorage UUID for anonymous customers; Shopify Customer Account merge folds anonymous history into `shopify:{customer_id}` on sign-in (`POST /v1/users/merge`). In-chat onboarding (photos + gender/DOB + combined height/waist; the `name` step was dropped 2026-05-17) collects profile data with full Skip support on photos and persistence across reloads. Post-onboarding analysis indicator + templated "what would you like to wear?" prompt close the flow. Pairing-trigger auto-augmentation ensures the engine routes attached-piece turns through `pairing_request`. Photos-card re-injection covers volume-loss recovery. Merchant admin (Polaris) shares the same Remix app.
> 3. **Vibe Engine** — `platform_core` Python service deployed to **Fly.io Mumbai** (`vibe-engine.fly.dev`, single shared-cpu-1x always-on with `vibe_data` persistent volume mounted at `/app/data`; `strategy = "immediate"` is the only Apps v2 deploy strategy compatible with single-machine + single-volume). The `vibe_storefront` channel bypasses the onboarding gate so anonymous / partial-profile customers can chat; the legacy `web` channel keeps the strict path. Observability: `aura_turn_total`, `aura_onboarding_endpoint_total`, and `aura_onboarding_image_bytes` carry `channel` labels for Vibe-vs-web slicing; `aura_user_merge_total` tracks outcomes by `status` (no channel label — merge is identity-side and channel-agnostic). Multi-tenant refactor (Phase C) deferred until Vibe customer-page UX validates.
>
