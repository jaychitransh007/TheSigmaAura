...
> **CURRENT SYSTEM (May 18, 2026, evening).** Aura ships as a **3-piece Shopify-hosted product**:
>
> 1. **Storefront** — `thesigmavibe.shop` (live on Shopify). Hosts 13,177 imported products from `catalog_enriched` (templatized descriptions + 20% markup CSV; the engine's `catalog_enriched` table itself holds 14,242 rows, frozen — see [`OPEN_TASKS.md`](OPEN_TASKS.md) § Frozen-catalog policy). 11,977 of 12,199 visible products have their `shopify_product_id` + `shopify_variant_ids` linked back to `catalog_enriched` as of 2026-05-18 evening (B.8 handle-suffix variant) — Buy Outfit / Buy Now resolves to real Shopify variant gids for ~98.2% of the catalog; the 222 stragglers fall through to the mock-id guard. Owns cart, checkout, payments (Razorpay), orders. India / INR only. Manual fulfillment to original retailers.
> 2. **Vibe (Shopify App)** — deployed to Vercel at `https://vibe-app-five.vercel.app` (Hobby plan, functions pinned to `bom1`). Multi-tenant: 2 tenants live as of May 18 (TheSigmaVibe prod + Vibe Test dev). **Installed in production on TheSigmaVibe** (`q8pery-95.myshopify.com`) on 2026-05-18 evening via Partner-Dashboard custom-distribution (D.P.1). Customer-facing chat runs at `<shop>/apps/vibe/style` via App Proxy with HMAC validation. **Customer-facing pages replicate the merchant's storefront chrome** (`/apps/vibe/*`): brand logo image (from `shop.brand.logo`), shop name, main-menu items (mixed-case, theme font), search / account / cart icons (cart-count badge auto-refreshes via `visibilitychange`). Theme variables (`--theme-color-primary`, `--theme-font-body`, `--theme-color-background`, `--theme-color-text`) injected at `:root` from per-tenant `theme_overrides` JSONB; canvas defaults to white with user messages in a soft `--user-message-bg` chip. **Two Vibe-owned menu items** ("Find your Vibe" → `/apps/vibe/style`, "Your Vibes" → `/apps/vibe/looks`) are idempotently injected into the merchant's `main-menu` linklist via Admin GraphQL `menuUpdate` and removed on uninstall (requires the `write_online_store_navigation` scope). Identity flows via localStorage UUID for anonymous customers; Shopify Customer Account merge folds anonymous history into `shopify:{customer_id}` on sign-in (`POST /v1/users/merge`) and propagates attribution fields (`acquisition_source` etc.) from alias to canonical. In-chat onboarding (photos + gender/DOB + combined height/waist; the `name` step was dropped 2026-05-17) collects profile data with full Skip support on photos, persistence across reloads, and a poll-loop self-heal that re-fires the analysis-start endpoints if the status stays at `not_started` (closes the silent-fail class behind a 49-minute stuck-spinner incident, PR #500). Merchant admin (Polaris) shares the same Remix app.
> 3. **Vibe Engine** — `platform_core` Python service deployed to **Fly.io Mumbai** (`vibe-engine.fly.dev`, single shared-cpu-1x always-on with `vibe_data` persistent volume mounted at `/app/data`; `strategy = "immediate"` is the only Apps v2 deploy strategy compatible with single-machine + single-volume). Multi-tenant catalog live (F.2.x + F.3 + F.4): `tenants` table, per-tenant `catalog_enriched` + `catalog_item_embeddings`, tenant-scoped retrieval RPC `match_catalog_item_embeddings_v2`. `PATCH /v1/tenants/{id}/theme-overrides` accepts and stores the merchant's captured theme metadata (font, primary color, background, text color, logo URL, shop name, main-menu items) which the customer-facing Vibe pages consume on every load. The `vibe_storefront` channel bypasses the onboarding gate so anonymous / partial-profile customers can chat; the legacy `web` channel keeps the strict path. Observability: `aura_turn_total`, `aura_onboarding_endpoint_total`, `aura_onboarding_image_bytes` carry `channel` labels; F.2.x sweep adds `aura_bootstrap_batch_total`, `aura_product_webhook_total`, `aura_enrichment_batch_total`, `aura_catalog_status_change_total`; theme inheritance adds `aura_theme_overrides_patch_total{outcome}`; May-18-evening sweep adds `aura_tenant_lookup_total{outcome}` + `aura_catalog_shopify_linkage_observed_total{bucket}` (PR #502) with `tenant_id` in the paired structured log. `aura_user_merge_total` tracks merge outcomes (no channel label — merge is identity-side). Multi-tenant refactor of the rest of the engine (Phase C) deferred until Vibe customer-page UX validates.
>
> **What sections below describe** is the previous *standalone Aura* surface — onboarding, web profile, web wardrobe, web chat. Those are deprecated. The product principles (memory-backed, wardrobe-first, explainable, confidence-visible) carry over and define how Vibe's customer pages should behave. **Authoritative source-of-truth: [`OPEN_TASKS.md`](OPEN_TASKS.md) § Locked infrastructure + Phase D / D.O plans.**
>
