...
Last updated: May 1, 2026 (Open Items Plan, ranking_bias wiring, reranker decision logging, stale doc sweep, onboarding step-count fix, dashboard SQL extraction, on-call runbook, structured JSON logging, Recent Signals timeline; staging verification: schema audit PASS, dependency report PASS, catalog 14,296 rows healthy, reranker calibration shipped with rank-position metric — found acquisition_source instrumentation regression in staging; Observability Hardening — all 12 items SHIPPED: trace-coverage fix, request_id middleware, /readyz + /version, token usage + cost, Prometheus /metrics, OpenTelemetry, PII redactor + GDPR delete, alert rules YAML, per-candidate traces, body sampling, retention sweep; Outfits Tab Theme Taxonomy SHIPPED — 8 canonical themes collapse the 14-bucket vocabulary explosion with regex word-boundary matching + per-process dedup'd unmapped-signal telemetry; Model Migration SHIPPED — Planner/Architect/User-Analysis upgraded to gpt-5.5; Visual-Evaluator/Image-Moderation moved to gpt-5-mini; 423 passed / 1 skipped)

Earlier: April 13, 2026 (Phase 15E-F complete + bugfix round: legacy surfaces removed, carousel polish + iteration stacking + swipe/keyboard, Like/Hide feedback with modal, restricted category word-boundary fix, outfit check 2-column card + fallback image save + evaluator title, intent routing fix for garment_evaluation, wishlist PascalCase column fix, send() race condition guards, composer redesign with + menu, attachment mutual exclusion fix)
