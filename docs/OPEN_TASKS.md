...
- [ ] **D.S.5.** **GDPR data pipeline.** Build the engine-side endpoints + delivery for what the D.S.4 webhook handlers currently only log. `POST /v1/users/{external_user_id}/data-export` walks the ~20 tables touching a user (`onboarding_profiles`, `onboarding_images`, `users`, `conversations`, `conversation_turns`, `user_wardrobe_items`, `saved_looks`, `virtual_tryon_images`, `catalog_interaction_history`, `confidence_history`, `policy_event_log`, `feedback_events`, `user_comfort_learning`, `*_snapshots`, …) and returns the customer's full dataset as JSON. `POST /v1/users/{external_user_id}/erase` deletes / anonymises the same set. Wire the webhook handlers to dispatch onto a queue (Vercel Cron + a `gdpr_requests` Prisma table) so the work is async and not bound to the 5s ack budget. **30-day SLA** under DPDPA / GDPR — until D.S.5 lands, requests must be processed manually from the `event=vibe_gdpr_*` Vercel Log Drain entries.
...
- [ ] **D.C.10.** Replace `window.confirm` / `alert` on Wardrobe + Looks pages with a branded confirm modal + toast. Native dialogs aren't styleable, block the main thread, and don't match the Luxe brand — flagged on the [#440 review](https://github.com/jaychitransh007/TheSigmaAura/pull/440). Affects delete confirms + fail-toast alerts in [apps.vibe.wardrobe.tsx](../vibe-app/app/routes/apps.vibe.wardrobe.tsx) and [apps.vibe.looks.tsx](../vibe-app/app/routes/apps.vibe.looks.tsx). Cheapest path: extract a small `<ConfirmDialog>` from the existing `AddPieceDialog` pattern + a portal-rendered `<Toast>`.
...
5. **D.S.5 — GDPR data pipeline.** Build engine `data-export` + `erase` endpoints + async queue so the D.S.4 webhook handlers do the work, not just log it. 30-day SLA per DPDPA/GDPR — until then process manually from Vercel Log Drain entries.
6. **D.C.10 — Replace `window.confirm` / `alert` on Wardrobe + Looks** (brand polish flagged on the #440 review).
7. Eventually: D.C.6 (try-on integration in Conversation + Looks), D.M.2–D.M.6 (richer merchant admin), D.P.2–D.P.3 (real-card e2e + listing assets), Phase C (engine multi-tenancy), Phase E (manual fulfillment SOP).
