...
> **⚠️ ARCHITECTURAL CONTEXT (May 18, 2026, evening).** Most of this document describes the *legacy standalone Aura web app* whose UI (`platform_core/ui.py`, the 3000-line server-rendered chat) is deprecated. The runtime ENGINE (planner / architect / composer / rater / try-on, in `agentic_application/` and `platform_core/`) remains valid and now runs on Fly.io. Vibe is installed in production on TheSigmaVibe as of this evening — see § Recently Shipped — May 18 evening in [`RELEASE_READINESS.md`](RELEASE_READINESS.md) for the install path.
>
> The current system has three deployments:
>
> 1. **Shopify storefront** — `thesigmavibe.shop`, 13,177 products live, India/INR. 11,977 of 12,199 visible products linked back to `catalog_enriched.shopify_product_id` via the handle-suffix B.8 variant (May 18 evening) — Add-to-Cart resolves to real Shopify variant gids for ~98.2% of the catalog.
> 2. **Vibe Shopify App** — Remix + Polaris on Vercel (`vibe-app-five.vercel.app`, functions pinned to `bom1`). Multi-tenant; 2 tenants live as of May 18 (TheSigmaVibe prod + Vibe Test dev). **Installed on TheSigmaVibe production** (`q8pery-95.myshopify.com`) on 2026-05-18 evening via Partner-Dashboard custom-distribution (Dev Dashboard apps issue `shpca_*` access tokens through the OAuth `install_custom_app` flow — newer prefix than the legacy `shpat_*`). Conversation page live at `/apps/vibe/style` at parity with the legacy `platform_core/ui.py` reference: composer with input + chip-in-pill attachment + `+` popover (image upload / wardrobe pick / wishlist pick), per-item size chips, PDP carousel with try-on hero, image proxy through `apps.vibe.api.tryon-image` for engine-served images. **Customer-facing pages `/apps/vibe/*` render a replicated merchant header** (logo image from `shop.brand.logo`, shop name, main-menu items, search / account / cart icons with live cart-count badge fetched from `/cart.js` and refreshed on `visibilitychange`). Theme inheritance: `font_body`, `color_primary`, `color_background`, `color_text`, `logo_url`, `shop_name`, `main_menu` are read from the merchant's active theme via Admin GraphQL (`shop { name brand { logo { image { url } } } } menus(first: 50)` + `config/settings_data.json` probe) on every admin home load + every `themes/update` webhook, PATCHed to engine `/v1/tenants/{id}/theme-overrides` (with a content-diff guard so unchanged loads skip the write), and injected as `--theme-*` CSS variables at `:root` via [theme-overrides.tsx](../vibe-app/app/components/theme-overrides.tsx). Two Vibe-owned menu items ("Find your Vibe", "Your Vibes") are idempotently injected into the merchant's `main-menu` linklist via `menuUpdate` (requires `write_online_store_navigation` scope) and removed by the `app/uninstalled` webhook. In-chat onboarding (photos + gender/DOB + combined height/waist, ft/in inputs; `name` step dropped 2026-05-17) with Skip on photos, full persistence, photos-card re-injection on volume loss, **analysis-trigger self-heal** (the poll loop re-fires `phase1` + `phase2` start endpoints when status sits at `not_started`, with an 8s cooldown — closes the silent-fail class behind a 49-min stuck-spinner incident, PR #500), and a post-onboarding "what would you like to wear?" prompt once analysis completes. Pairing-trigger auto-augmentation rewrites attached-piece turns to "Build an outfit around this attached piece" so the engine routes them through `pairing_request`. Identity: anonymous localStorage UUID; Shopify Customer Account merge wired (D.S.3b) with IDOR-guarded `op=merge`. Structured JSON logs to Vercel via [logger.server.ts](../vibe-app/app/lib/logger.server.ts) — onboarding (`vibe_init_outcome`, `vibe_onboarding_stage_reach`, `vibe_onboarding_endpoint_failed`), catalog (`vibe_bootstrap_tick`, `vibe_cron_daily_sync_*`, `vibe_product_webhook_*`), and theme/menu (`vibe_theme_overrides_refresh`, `vibe_menu_injection_outcome`, `vibe_navigation_menu_{lookup,update}_failed`, `vibe_tenant_lookup_failed`). Wardrobe / Looks / Outfit Check pages still pending.
> 3. **Vibe Engine** — `platform_core` Python service on **Fly.io Mumbai** (`vibe-engine.fly.dev`, shared-cpu-1x always-on with `vibe_data` persistent volume at `/app/data` so onboarding photos + wardrobe tiles + try-on renders survive deploys; `strategy = "immediate"` — the only Apps v2 strategy compatible with single-machine + single-volume). Multi-tenant catalog live (F.2.x + F.3 + F.4): `tenants` table with opaque `tenant_id`, per-tenant `catalog_enriched` + `catalog_item_embeddings`, tenant-scoped retrieval RPC `match_catalog_item_embeddings_v2`. `PATCH /v1/tenants/{id}/theme-overrides` accepts the merchant's captured theme metadata + main-menu shape and stores them on the `tenants.theme_overrides` JSONB column. `vibe_storefront` channel bypasses the onboarding gate for App Proxy chat; `web` channel keeps the legacy strict path. Identity-merge endpoint `POST /v1/users/merge` reassigns conversation + history rows from the anonymous localStorage UUID to `shopify:{customer_id}` on sign-in **and now also propagates `acquisition_source` / `acquisition_campaign` / `referral_code` / `icp_tag` from the alias's `onboarding_profiles` to the canonical's when the canonical is at defaults** (PR #503 — without this fix Panel 01 would show every merged customer as `acquisition_source="unknown"` forever). Prometheus: `aura_turn_total{channel}`, `aura_user_merge_total{status}`, `aura_onboarding_endpoint_total{endpoint, status, channel}`, `aura_onboarding_image_bytes{category, channel}`, F.2.x sweep (`aura_bootstrap_batch_total`, `aura_product_webhook_total`, `aura_enrichment_batch_total`, `aura_catalog_status_change_total`, `aura_outfit_item_variant_ids_missing_total`), theme inheritance (`aura_theme_overrides_patch_total{outcome}`), tenant + catalog linkage (PR #502: `aura_tenant_lookup_total{outcome=created|existing|error}`, `aura_catalog_shopify_linkage_observed_total{bucket}` + structured log `catalog_shopify_linkage_observed` with `tenant_id`). `GET /v1/onboarding/status/{user_id}` is consumed by the Vibe client with three-way return semantics (`OnboardingStatus | null | undefined`). Orchestrator no longer carries forward the previous turn's `attached_item` (PR #430). Multi-tenancy refactor of the rest of the engine (Phase C, beyond catalog) deferred until Vibe customer pages validate.
>
...
- pairing pipeline end-to-end working on the Vibe storefront — auto-augmentation in the Vibe action ("Build an outfit around this attached piece") forces the planner into `pairing_request` whenever any attachment (image upload / wardrobe pick / wishlist pick) is set; anchor injection + role stripping live; engine-side carry-forward of the previous turn's `attached_item` was removed (PR #430) so reloading a conversation no longer silently re-anchors on a stale wardrobe piece
...
- **Multi-tenant catalog read path** (F.2.x, May 18 2026): `tenants` table + opaque `tenant_id` per shop, `catalog_enriched.tenant_id` + `catalog_item_embeddings.tenant_id` partition keys, new RPC `match_catalog_item_embeddings_v2(p_tenant_id, query_embedding, ...)` raises on empty tenant rather than silently leaking cross-tenant rows. HNSW iterative scan with `max_scan_tuples=20000` keeps tenant filter cheap (validated 185ms on 0.035% selectivity in spike F.2.1).
- **Idempotent install-time bootstrap** (F.2.2): `CatalogBootstrapService.process_products` checks existence per product; cache-hit refreshes mutable metadata only (no LLM call), cache-miss embeds + inserts. Tenant-prefixed `product_id` (`{tenant_id}:{shopify_product_id}`) avoids global UNIQUE collision when two tenants import the same Shopify GID. Browser-driven chunk processor in vibe-app paginates Shopify Admin GraphQL one page at a time and stays under Vercel's 60s function ceiling.
- **Vision attribute enrichment via Batch API** (F.2.2b): `CatalogVisionEnrichmentService.submit_pending_for_tenant` ships pending rows to OpenAI's `/v1/responses` Batch API. `poll_and_ingest` downloads completed output + writes 34 PascalCase columns to `catalog_enriched` + 8 lowercase filter columns to `catalog_item_embeddings`. Cost-bearing invariant SQL-enforced (`row_status='pending_enrichment' AND vision_batch_id IS NULL`) so already-enriched rows never re-run. Image-less rows skipped at submit time to prevent infinite-retry loops.
- **Product webhooks** (F.4): `products/{create,update,delete}` handlers in vibe-app forward to engine. Soft-delete only (preserves vision enrichment cost; revivable). Topic-aware revival — only `products/create` clears `deleted_at`, protecting against out-of-order `products/update` retries. `catalog_enriched.available_for_sale` + `deleted_at` columns mirrored to embeddings; retrieval RPC filters `IS NOT FALSE`.
- **Daily catalog cron** (F.3): Vercel cron at 19:30 UTC enumerates ready tenants, walks each merchant's Shopify Admin GraphQL with `updated_at:>=30h` filter, forwards to engine `bootstrap-batch` with `revive_soft_deleted=true` (cron-walks revive prior soft-deletes), then triggers `enrichment/submit-pending-all` + `enrichment/poll-all`. Soft 45s wall budget; tenants ordered oldest-`last_sync_at`-first so partial runs make forward progress on stale shops.
