...
> **⚠️ ARCHITECTURAL CONTEXT (May 17, 2026).** Most of this document describes the *legacy standalone Aura web app* whose UI (`platform_core/ui.py`, the 3000-line server-rendered chat) is deprecated. The runtime ENGINE (planner / architect / composer / rater / try-on, in `agentic_application/` and `platform_core/`) remains valid and now runs on Fly.io.
>
> The current system has three deployments:
>
> 1. **Shopify storefront** — `thesigmavibe.shop`, 13,177 products live, India/INR.
> 2. **Vibe Shopify App** — Remix + Polaris on Vercel (`vibe-app-five.vercel.app`, functions pinned to `bom1`). Conversation page live at `/apps/vibe/style` at parity with the legacy `platform_core/ui.py` reference: composer with input + chip-in-pill attachment + `+` popover (image upload / wardrobe pick / wishlist pick), per-item size chips, PDP carousel with try-on hero, image proxy through `apps.vibe.api.tryon-image` for engine-served images. In-chat onboarding (photos + gender/DOB + combined height/waist, ft/in inputs; `name` step dropped 2026-05-17) with Skip on photos, full persistence, photos-card re-injection on volume loss, and a post-onboarding "what would you like to wear?" prompt once analysis completes. Pairing-trigger auto-augmentation rewrites attached-piece turns to "Build an outfit around this attached piece" so the engine routes them through `pairing_request`. Identity: anonymous localStorage UUID; Shopify Customer Account merge wired (D.S.3b) with IDOR-guarded `op=merge`. Structured JSON logs to Vercel (`vibe_init_outcome`, `vibe_merge_outcome`, `vibe_merge_identity_mismatch`, `vibe_turn_pairing_rewrite`) provide per-request greppability. Wardrobe / Looks / Outfit Check pages still pending.
> 3. **Vibe Engine** — `platform_core` Python service on **Fly.io Mumbai** (`vibe-engine.fly.dev`, shared-cpu-1x always-on with `vibe_data` persistent volume at `/app/data` so onboarding photos + wardrobe tiles + try-on renders survive deploys; `strategy = "immediate"` — the only Apps v2 strategy compatible with single-machine + single-volume). `vibe_storefront` channel bypasses the onboarding gate for App Proxy chat; `web` channel keeps the legacy strict path. Identity-merge endpoint `POST /v1/users/merge` reassigns conversation + history rows from the anonymous localStorage UUID to `shopify:{customer_id}` on sign-in. Per-route channel-labelled Prometheus counters (`aura_onboarding_endpoint_total`, `aura_onboarding_image_bytes`) shipped 2026-05-17 alongside `aura_turn_total{channel}` and `aura_user_merge_total{status}`. `GET /v1/onboarding/status/{user_id}` is consumed by the Vibe client with three-way return semantics (`OnboardingStatus | null | undefined`) — 404 means confirmed new customer, errors fall back to "assume profile exists" so engine wobble doesn't re-onboard returning customers. Orchestrator no longer carries forward the previous turn's `attached_item` (PR #430). Multi-tenancy refactor (Phase C) deferred until Vibe customer pages validate.
>
