...
const details = {
  ...
  orchestrator: {
    ...
    desc: "Central intent-runtime pipeline that now combines planner dispatch with dedicated handlers for wardrobe-first occasion answers, outfit check, shopping decision, pairing, style discovery, explanation, capsule/trip planning, and virtual try-on. Uploaded garment images are classified as wardrobe-image or catalog-image anchors before routing. Recommendation-pipeline turns continue through Architect → Search → Assembler → Evaluator → Formatter → Virtual Try-On, while handler-backed intents can short-circuit with deterministic business logic. Latency tracked per stage via time.monotonic().",
    items: [
      "1. User Context Builder — loads profile via OnboardingGateway",
      "2. Planner Input Builder — conversation memory + attachment context",
      "3. Copilot Planner [AGENT] — intent + action dispatch (gpt-5.4)",
      "4. Deterministic overrides — pairing, outfit-check, source-preference, catalog CTA",
      "5. Dedicated handlers — outfit check, shopping decision, pairing, style, capsule/trip",
      "6. Outfit Architect [AGENT] — LLM plan for recommendation-pipeline turns",
      "7. Catalog Search / Assemble / Evaluate — retrieval + ranking",
      "8. Response Formatter [TOOL] — source-aware messaging + metadata",
      ...
      "Pairing image intake distinguishes wardrobe_image vs catalog_image anchors",
      "Pipeline failure → non-empty fallback response (no silent blank turns)",
    ]
  },
  context_resolver: {
