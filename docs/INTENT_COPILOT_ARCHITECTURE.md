...
> **DEPLOYMENT CONTEXT (May 18, 2026, evening).** This document still accurately describes the engine's intent taxonomy, agent topology, and turn-execution pipeline (planner → architect → composer → rater → try-on). What's different from earlier drafts:
>
...
> - Callers: the **Vibe Shopify App** at `vibe-app-five.vercel.app` proxies customer chat to the engine through the App Proxy. **Vibe is installed in production on TheSigmaVibe** (`q8pery-95.myshopify.com`) as of 2026-05-18 evening (D.P.1). Turns are tagged with `channel="vibe_storefront"`, which bypasses the onboarding gate so anonymous / partial-profile customers can chat. The legacy `web` channel keeps the strict gate path for backward compatibility.
> - Identity: anonymous customers use a localStorage UUID; signed-in customers merge to `shopify:{customer_id}` via `POST /v1/users/merge` (D.S.3b). The merge endpoint also propagates attribution fields (`acquisition_source` / `acquisition_campaign` / `referral_code` / `icp_tag`) from the alias's `onboarding_profiles` to the canonical's when canonical is at defaults (PR #503, 2026-05-18 evening — without this fix, every merged `vibe_storefront` customer would show up in Panel 01 as `unknown` forever).
...
> - Onboarding-analysis trigger self-heal (PR #500, 2026-05-18 evening): the Vibe poll loop now retries `POST /v1/onboarding/analysis/start-phase{1,2}` when the engine status sits at `not_started` (8s cooldown). Closes a silent-fail class behind a 49-minute stuck-spinner incident — the original fire-and-forget trigger only caught network errors, so HTTP 4xx/5xx responses silently dropped on the floor.
> - Observability: all Vibe-callable engine routes (turn loop + 7 onboarding endpoints) emit `channel`-labelled Prometheus counters and structured `event=...` logs as of PR #436. The F.2.x catalog sweep (#475/#476), theme-inheritance work (#498), and the May-18-evening sweep (#502: `aura_tenant_lookup_total`, `aura_catalog_shopify_linkage_observed_total` + structured log `catalog_shopify_linkage_observed`) add further per-domain counters. Vibe-side Remix actions emit JSON-to-stdout events to Vercel Logs across onboarding (`vibe_init_outcome`, `vibe_merge_outcome`, `vibe_turn_pairing_rewrite`, plus #501 `vibe_onboarding_stage_reach{stage}` and `vibe_onboarding_endpoint_failed{endpoint}`), catalog (`vibe_bootstrap_tick`, `vibe_cron_daily_sync_*`, `vibe_product_webhook_*`), and theme/menu (`vibe_theme_overrides_refresh`, `vibe_menu_injection_outcome`, `vibe_navigation_menu_{lookup,update}_failed`).
> - Multi-tenant catalog read path is **live** (F.2.x + F.3 + F.4, 2026-05-18): `tenants` table, per-tenant `catalog_enriched` + `catalog_item_embeddings`, tenant-scoped retrieval RPC `match_catalog_item_embeddings_v2`. Two tenants live. Full Phase C engine-wide multi-tenancy (every cache key / orchestrator slot scoped to `tenant_id`) still deferred until Vibe customer pages validate at volume.
> - Customer-facing Vibe pages (`/apps/vibe/*`) render the merchant's storefront chrome (replicated header with brand logo + main-menu items + search/account/cart icons; theme colors + font injected as CSS variables; per-tenant `theme_overrides` stored on the `tenants` row and refreshed via `PATCH /v1/tenants/{id}/theme-overrides` on every admin home load + every `themes/update` webhook). Two Vibe-owned menu items ("Find your Vibe", "Your Vibes") are idempotently injected into the merchant's `main-menu` linklist; requires the `write_online_store_navigation` scope.
>
