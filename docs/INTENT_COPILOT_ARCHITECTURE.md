...
> **DEPLOYMENT CONTEXT (May 18, 2026).** This document still accurately describes the engine's intent taxonomy, agent topology, and turn-execution pipeline (planner → architect → composer → rater → try-on). What's different from earlier drafts:
>
...
> - Observability: all Vibe-callable engine routes (turn loop + 7 onboarding endpoints) emit `channel`-labelled Prometheus counters and structured `event=...` logs as of PR #436. The F.2.x catalog sweep (#475/#476) and theme-inheritance work (#498) add further per-domain counters (`aura_bootstrap_batch_total`, `aura_product_webhook_total`, `aura_enrichment_batch_total`, `aura_catalog_status_change_total`, `aura_theme_overrides_patch_total`). Vibe-side Remix actions emit JSON-to-stdout events to Vercel Logs across onboarding (`vibe_init_outcome`, `vibe_merge_outcome`, `vibe_merge_identity_mismatch`, `vibe_turn_pairing_rewrite`), catalog (`vibe_bootstrap_tick`, `vibe_cron_daily_sync_*`, `vibe_product_webhook_*`), and theme/menu (`vibe_theme_overrides_refresh`, `vibe_menu_injection_outcome`, `vibe_navigation_menu_{lookup,update}_failed`).
