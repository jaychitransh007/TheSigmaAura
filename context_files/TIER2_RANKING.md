...
Configured in:
- `strictness_profiles` in `catalog_enrichment/tier2_rules_v1.json`
...
python3 scripts/rank_outfits.py \
  ...
  --output out/ranked_outfits_safe.csv \
  --explain out/ranked_outfits_safe_explainability.json
```
...
python3 scripts/rank_outfits.py \
  ...
  --output out/ranked_outfits_bold.csv \
  --explain out/ranked_outfits_bold_explainability.json
```
