...
class AgenticApplicationTests(unittest.TestCase):
    ...
    def test_outfit_architect_drops_kurta_paired_direction(self) -> None:
        """May 3 2026 hard pairing rule — a paired direction whose top role
        is kurta or tunic is invalid by construction (no catalog-side
        kurta_pant / pyjama / churidar). The parser drops it."""
        context = CombinedContext(
            user=UserContext(user_id="u1", gender="male"),
            live=LiveContext(user_need="Wedding traditional outfit"),
        ...
        mock_response.output_text = json.dumps({
            ...
            "retrieval_count": 5,
            "directions": [
                {  # Valid: complete kurta_set
                    "direction_id": "A",
                    "direction_type": "complete",
                    "label": "Traditional kurta_set",
                    "queries": [{
                        "query_id": "A1",
                        "role": "complete",
                        "hard_filters": {"garment_subtype": "kurta_set"},
                        "query_document": "GARMENT_REQUIREMENTS:\n- GarmentCategory: complete outfit\n- GarmentSubtype: kurta_set\n",
                    }],
                },
                {  # INVALID: paired with kurta top
                    "direction_id": "B",
                    "direction_type": "paired",
                    "label": "Kurta and trouser pairing",
                    "queries": [
                        {
                            "query_id": "B1",
                            "role": "top",
                            "hard_filters": {"garment_subtype": "kurta"},
                            "query_document": "GARMENT_REQUIREMENTS:\n- GarmentCategory: top\n- GarmentSubtype: kurta\n",
                        },
                        {
                            "query_id": "B2",
                            "role": "bottom",
                            "hard_filters": {"garment_subtype": "trouser"},
                            "query_document": "GARMENT_REQUIREMENTS:\n- GarmentCategory: bottom\n- GarmentSubtype: trouser\n",
                        },
                    ],
                },
                {  # INVALID: three_piece with kurta top — also dropped
                    "direction_id": "C",
                    "direction_type": "three_piece",
                    "label": "Kurta + trouser + nehru jacket",
                    "queries": [
                        {
                            "query_id": "C1",
                            "role": "top",
                        ...
                        },
                        {
                            "query_id": "C3",
                            "role": "outerwear",
                            "hard_filters": {"garment_subtype": "nehru_jacket"},
                            "query_document": "GARMENT_REQUIREMENTS:\n- GarmentSubtype: nehru_jacket\n",
                        },
                    ],
                },
            ],
        ...
        self.assertEqual("A", plan.directions[0].direction_id)
        self.assertEqual("complete", plan.directions[0].direction_type)

    def test_outfit_architect_kurta_paired_text_only_also_dropped(self) -> None:
        """The kurta detector also catches tops whose subtype only appears
        in the query_document text, not the structured hard_filters."""
        context = CombinedContext(
            user=UserContext(user_id="u1", gender="male"),
            live=LiveContext(user_need="Wedding"),
            hard_filters={"gender_expression": "masculine"},
        )

        mock_response = Mock()
        mock_response.output_text = json.dumps({
            "resolved_context": {"occasion_signal": "wedding_ceremony", "is_followup": False},
            "retrieval_count": 5,
            "directions": [
                {  # paired top without subtype hard_filter, but text mentions kurta
                    "direction_id": "A",
                    "direction_type": "paired",
                    "label": "Mystery kurta paired",
                    "queries": [
                        {
                            "query_id": "A1",
                            "role": "top",
        ...
        with patch("agentic_application.agents.outfit_architect.get_api_key", return_value="x"), patch(
            ...
            architect = OutfitArchitect()
            with self.assertRaises(RuntimeError):
                # All directions dropped → empty plan → architect raises
                architect.plan(context)
