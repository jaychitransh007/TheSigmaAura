    ...
    def _load_or_generate_embeddings(self, documents, config, cache_path: Path):
        ...
        if cache_path.exists():
            try:
                ...
                # text hash AND was generated by the current model +
                # dimensions. Compute hashes lazily inside this branch
                # so a missing cache file (the common path) does no work.
                wanted: Dict[str, str] = {
                    d.product_id: hashlib.sha256(d.document_text.encode("utf-8")).hexdigest()
                    for d in documents
                }
                valid = True
                invalid_reason = ""
                for pid, want_hash in wanted.items():
                    ...
                    if rec is None:
                        valid = False
                        invalid_reason = f"missing product_id={pid}"
                        break
                    ...
                    if have_hash != want_hash:
                        valid = False
                        invalid_reason = f"text hash drift on product_id={pid}"
                        break
                if valid:
                    logger.info(
