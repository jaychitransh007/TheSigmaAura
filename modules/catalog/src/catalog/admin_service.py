...
class CatalogAdminService:
    ...
    def _load_or_generate_embeddings(self, documents, config, cache_path: Path):
        ...
        "Fresh" = cache covers every input document_text exactly AND was
        generated by the same model + dimensions the current config asks
        for. Cache is keyed by (product_id, document_text hash) per row;
        any drift (new enriched data, new document_builder code, new
        embedding model, or different vector size) invalidates the cache
        and triggers a regenerate.
        """
        if cache_path.exists():
            try:
                ...
                # Validate cache covers every wanted doc with matching
                # text AND was generated by the current model +
                # dimensions. Direct string comparison instead of SHA256:
                # the document_text is already in memory on both sides,
                # hashing 14K strings just to compare them is wasteful.
                valid = True
                invalid_reason = ""
                for d in documents:
                    rec = cached_records.get(d.product_id)
                    if rec is None:
                        valid = False
                        invalid_reason = f"missing product_id={d.product_id}"
                        break
                    if rec.model != config.embedding_model:
                        valid = False
                        invalid_reason = f"model drift (cache={rec.model}, config={config.embedding_model})"
                        break
                    if rec.dimensions != config.embedding_dimensions:
                        valid = False
                        invalid_reason = f"dimension drift (cache={rec.dimensions}, config={config.embedding_dimensions})"
                        break
                    if rec.document_text != d.document_text:
                        valid = False
                        invalid_reason = f"text drift on product_id={d.product_id}"
                        break
                if valid:
                    logger.info(
                    ...
                    logger.info(
                        "resync_catalog_embeddings: cache %s is stale (%s); regenerating",
